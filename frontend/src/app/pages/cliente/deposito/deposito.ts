...
export class Deposito implements OnInit {
  ...
  confirmarDeposito(): void {
    ...
    const saldoEsperado = saldoAnterior.plus(valor).toFixed(4);
    this.changeDetector.detectChanges();

    this.contaService.depositar(this.conta.numeroConta, valor.toFixed(2)).subscribe({
      next: resposta => {
        this.formulario.reset();
        this.mensagemSucesso = resposta.mensagem || 'Depósito realizado com sucesso.';
        this.changeDetector.detectChanges();
  ...
  private mensagemParaErro(erro: { status?: number; error?: { mensagem?: string } }): string {
    ...
    return erro.error?.mensagem || 'Não foi possível realizar o depósito.';
  }
}
